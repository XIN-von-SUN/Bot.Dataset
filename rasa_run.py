...
def rasa_nlu_loop(data_file, write_file, test_data, dimensions):
    ...
    for i in range(len(dimensions)):  # range(len(dimensions))
        ...
        if [dim1,dim2,dim3,dim4,dim5,dim6] not in model_set:
            ...
            '''
            # here appears sth wrong lead to duplication
            if os.path.exists('./rasa_result/rasa_eval_result.csv'):
                result_orig = pd.read_csv('./rasa_result/rasa_eval_result.csv', index_col=0)
                result = pd.concat([result_orig, result]).reset_index(drop=True)
                result.to_csv('./rasa_result/rasa_eval_result.csv')
            else:
                result.to_csv('./rasa_result/rasa_eval_result.csv')
            '''
